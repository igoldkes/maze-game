...
pub fn load_trap_item_texture(path: &str) -> Texture2D {
    match fs::read(path) {
        Ok(bytes) => Texture2D::from_file_with_format(&bytes, None),
        Err(_) => build_fallback_trap_texture(),
    }
}

pub fn load_path_marker_texture(path: &str) -> Texture2D {
    match fs::read(path) {
        Ok(bytes) => Texture2D::from_file_with_format(&bytes, None),
        Err(_) => build_fallback_path_marker_texture(),
    }
}
...
pub fn load_player_heart_texture(path: &str) -> Texture2D {
    match fs::read(path) {
        Ok(bytes) => Texture2D::from_file_with_format(&bytes, None),
        Err(_) => build_fallback_player_heart_texture(),
    }
}
...
fn build_fallback_trap_texture() -> Texture2D {
    const W: u32 = 64;
    const H: u32 = 64;
    let mut img = Image::gen_image_color(W as u16, H as u16, Color::from_rgba(0, 0, 0, 0));
    let cx = W as f32 * 0.5;
    let cy = H as f32 * 0.5;
    let fill = Color::from_rgba(190, 140, 70, 255);
    let edge = Color::from_rgba(60, 35, 0, 255);
    for y in 0..H {
        for x in 0..W {
            let dx = (x as f32 - cx).abs();
            let dy = (y as f32 - cy).abs();
...
fn build_fallback_player_heart_texture() -> Texture2D {
    const W: u32 = 64;
    const H: u32 = 64;
    let mut img = Image::gen_image_color(W as u16, H as u16, Color::from_rgba(0, 0, 0, 0));
    let cx = W as f32 * 0.5;
...
fn build_fallback_path_marker_texture() -> Texture2D {
    const W: u32 = 64;
    const H: u32 = 64;
    let mut img = Image::gen_image_color(W as u16, H as u16, Color::from_rgba(0, 0, 0, 0));
    let cx = W as f32 * 0.5;
    let cy = H as f32 * 0.5;
    let fill = Color::from_rgba(255, 250, 220, 255);
    let edge = Color::from_rgba(255, 255, 255, 255);
    for y in 0..H {
        for x in 0..W {
            let dx = (x as f32 - cx).abs();
            let dy = (y as f32 - cy).abs();
